...
"""
执行用时 :72 ms, 在所有 python3 提交中击败了97.89%的用户
...
class Solution:
    def addTwoNumbers(self, l1: ListNode, l2: ListNode) -> ListNode:
        ...
        if l1.next==l2.next==None:
            ...
            l1.next.val += carry
            l.next = self.addTwoNumbers(l1.next, l2.next) # 交由递归处理
        else: # l1, l2其中一个next为None
            if not carry:
                l.next = (l1.next or l2.next) # 直接饭
            if carry:
                (l1.next or l2.next).val += carry
                l.next = self.addTwoNumbers((l1.next or l2.next), None) # 交由递归处理
        return l
